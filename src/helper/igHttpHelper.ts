...
// Audio Headers
const audioHeaders = (referer: string) => {
    const randomString = (length: number) => {
        const chars = 'abcdefghijklmnopqrstuvwxyz0123456789';
        let result = '';
        for (let i = 0; i < length; i++) { result += chars.charAt(Math.floor(Math.random() * chars.length)); }
    ...
    return {
        ...
        'X-Csrftoken': csrf,
        'X-Ig-App-Id': '936619743392459',
        'X-IG-WWW-Claim': Math.random() < 0.5 ? '0' : `hmac.${randomString(32)}`,
        'X-Instagram-AJAX': Math.floor(Math.random() * (99999999 - 10000000 + 1) + 10000000).toString(16).padStart(8, '0'),
        'X-Requested-With': 'XMLHttpRequest',
        'X-Web-Session-Id': `${randomString(6)}:${randomString(6)}:${randomString(6)}`,
    };
}
...
// Export
export { rpHeaders, rpPayload, wpiHeaders, feedHeaders, audioHeaders };